...
| Fig. 14 — Partition Node Element data collection (p.36) | Fig. 23 | `structural` | `spec+fixture` | partial | done | `LsgCodecs.kt:386` | 9.5 inserts `BBoxF32: Reserved Field` on the **main** path and puts `BBoxF32: Transformed BBox` on a branch guarded `(Partition Flags & 0x00000001) == 0`. Exactly one of the two is written, so the byte count matches v10 in both branches — but the *identity* of the box differs. Finding 2 |
...
Contradictions between the library's JT 9 path and the 9.5 document: **4** found, **3 closed**
by package P2 / issue #11 (findings 1, 3, 6); **1 open** — finding 2, the Partition Node
bounding-box identity flip, deliberately left to its own package so two independent changes do
not share a diff.
