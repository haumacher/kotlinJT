...
**P8 — the two 9.5-only compressed data packets** (package H's findings H-2 and H-3, and with them
G-2/G-3). `encoding.Int32CdpMk1` (§8.1.1) and `encoding.Float64Cdp` (§8.1.3) now decode and
re-serialize byte-identically, together with the §8.1.13/§8.1.14/§8.1.15 NURBS collections in
their 9.5 form (`CompressedCurveData.read95`). The load-bearing design decision is that **9.5's
Mk. 1/Mk. 2 split has no wire signal at all** — the figure notation binds it statically, per field
— so the two packets are unrelated types with no common supertype and no auto-detecting entry
point, and the acceptance is not "both decode" but "each refuses the other's bytes by name". Four
places where the document stops short are refusals rather than guesses (a Mk. 1 chopper, a Float64
bitlength or chopper, and a two-table Float64 packet with a live CodeText, which Figure 226 gives
no `Next Context` field to switch with); two readings are marked as inference in code and record
(a Mk. 1 escape outside table 0 emits nothing; a Float64 Null CODEC packs two words per value,
low word first). **Every row is `spec`** — the corpus has no file carrying either packet — but the
corpus is not idle: 663 real Mk. 2 arithmetic packets are re-framed into the Mk. 1 layout and must
decode identically, and all 684 are refused by name when offered to the Mk. 1 reader in their own
framing. The JT 9 wireframe rep element, the JT B-Rep topology streams and the ULP `params1`
collection stay opaque with named blockers: they now lack their own framing, not a codec.

**P9 — the 9.5 Meta Data / PMI segment** (package F, findings F-1, F-4, F-5, F-8, F-10, F-12).
The last package of the roadmap, and the one where the two documents diverge most: **v10 deleted
the typed-PMI-entity family**, so 9.5's PMI Manager is not the v10 element with extra fields but
an older, larger wire format sharing its Object Type ID. It is implemented as *a second element
family beside the v10 one* (`meta/Pmi95Elements.kt`, `meta/Pmi95Codecs.kt`), never as version
branches inside the v10 codec — the reason is F-5: PMI Associations permute five `I32` words, so
a v10-order read of a 9.5 association consumes exactly the right byte count and yields silently
wrong semantics, which no length check, byte count or round-trip can catch. All of Figures 136–170
decode and round-trip byte-identically, including Figure 137's thirteen typed collections in the
figure's own order and the 9.5-only PMI 3D Data. Three deltas that desynchronize rather than
mislead are pinned by their own tests: single-byte `String` PMI strings and font names (F-8), the
`VecI32` character set, and the Hidden Flag's third state — *absent* below PMI version 7 (F-10).
Two places the document argues with itself are named rather than resolved by preference: the
`TextureBinding == 1` branch of Figure 170 (figure says `I16 : Reserved Field`, prose says
`VecF32: Texture Coords` — the prose is followed, every occurrence raises
`PMI_POLYGON_TEXTURE_BINDING_UNSETTLED`) and 9.5's own Compressed CAD Tag Data, which is framed
from its Data Length and kept verbatim because §8.1.16's 9.5 form is a different collection and
belongs to the compression package — `encoding/CadTagData.kt` was **not** touched. **F-4 is the
package's doctrine exercise**: DESIGN.md delta 36 read Figure 126's guard the wrong way round;
both references in fact enclose the `VecF32` in it, and NX 10.5 writes it anyway, so the field is
now nullable, both forms round-trip, and the 9.5 reader *arbitrates* between them from the body's
exact extent — possible precisely because F-2 holds and the 9.5 element ends at its font loop.
Every row is `spec`: no fixture in the corpus carries a JT 9 PMI Manager, and `Pmi95FixtureTest`
is the hook that fires on the first, checking among other things that **every gated field's
presence agrees with the version pair that gates it**.
...
Read/Write describe the **JT 9 path**. `n/a: writer emits v10` is the *authoring* entry
throughout — `writeJt` authors no §7.2.6 segment in any generation — and `done (re-ser)` is the
achievable Write target: byte-faithful re-serialization of what was read, which is what Layer 1
losslessness on 9.5 input needs.

**Package P9 implemented this package.** The whole of Figures 136–170 now decodes into its own
element family (`meta/Pmi95Elements.kt`, `meta/Pmi95Codecs.kt`) beside the v10 one, because v10
deleted the thirteen typed PMI-entity collections rather than modifying them. Evidence is `spec`
for every 9.5-only and structural row: **no fixture in the corpus carries a JT 9 PMI Manager**,
so `Pmi95FixtureTest` is the auto-discovering hook that fires on the first and skips visibly
until then. Rows that the NIST 10.5 file exercises through the *shared* codecs carry
`spec+fixture`.

| 9.5 unit | v10 counterpart | Delta | Evidence | Read | Write | Code | Notes |
|---|---|---|---|---|---|---|---|
| Fig. 133 — Meta Data Segment data collection (p.162) | Fig. 107 | `identical` | `spec+fixture` | done | done (re-ser); n/a: writer emits v10 | `MetaDataDocument.kt:91` | Segment Header + repeated Meta Data Element; neither reference draws the end-of-elements marker or the trailing Property Table. §7.1.3.2.3 (p.32) puts the ZLIB header on the **first element only**, so after Layer-0 decompression the element stream is plain `I32 length + GUID + body` exactly as in v10. F-9. Tests: `MetaDataDocumentTest`, `Pmi95DocumentTest` |
| §7.1.3.2.2 Element Header (p.31) *(framing dependency)* | §5 Element Header | `identical` | `spec+fixture` | done | done (re-ser) | `MetaDataCodecs.kt:40` | `GUID Object Type ID + UChar Object Base Type + I32 Object ID`; Table 4 base type 9 = JtBase, as the code asserts. Object Type IDs for both meta elements are byte-identical to v10 |
| Fig. 134 — Property Proxy Meta Data Element data collection (p.163) | Fig. 108 | `widths` | `spec+fixture` | done | done (re-ser); n/a: writer emits v10 | `MetaDataCodecs.kt:139`, `:23` | Only delta: `I16 Version Number` (9.5) vs v10 `U8`. Same key loop, same guard wording, same four value branches. F-6. Test: `MetaDataDocumentTest.propertyProxyCarriesEveryTable53ValueType` |
| Table (unnumbered) — Property Value Type values (p.163) | Table 53 | `identical` | `spec+fixture` | done | done (re-ser) | `MetaDataCodecs.kt:156` | 9.5's set is exactly `{0 Unknown, 1 MbString, 2 I32, 3 F32, 4 Date}` — no 9.5-only value type. F-7 |
| Fig. 135 — Date Property Value data collection (p.164) | Fig. 109 | `identical` | `spec` | done | done (re-ser) | `MetaDataCodecs.kt:110` | Six `I16` fields, same order, same names, no guards. *(No fixture writes a Date value in either generation — exercised only by the hand-built per-figure test.)* |
| Fig. 136 — PMI Manager Meta Data Element data collection (p.166) | Fig. 110 | `structural` (nine independent differences) | `spec` | **done** | done (re-ser); n/a: writer emits v10 | `Pmi95Codecs.kt:readPmi95Manager`, `MetaDataDocument.kt:206` | The prologue is `I16 Version + I16 PMI Version + I16 Reserved`; then Figure 137's block, associations, user attributes, string table, `[PMI Version > 5]` model views + generic entities, `[PMI Version > 7]` CAD tags, `[Version > 1]` per-view properties + polygon data + fonts, and **the element ends** — F-2 stands, `PMI_MANAGER_TAIL_UNDOCUMENTED` is untouched. Guard extents read from the rendered p.166 image. DESIGN.md deltas 48–50. Tests: `Pmi95DocumentTest.figure136DecodesEveryCollectionAndRoundTrips`, `.elementVersionOneStopsBeforeTheTailBlock`, `.aNinePointFiveBodyIsNotReadAsAV10Manager` |
| Value set — PMI Version Number 3…8 (p.166) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt` (`PMI_VERSIONS`, `Pmi95Gates`) | Six PMI generations gate six branches. A value outside 3…8 (or an element version outside 1…2) refuses by name — the guards it drives would otherwise be guesswork. Test: `Pmi95DocumentTest.aV9PmiManagerWithAnUndocumentedVersionNumberRefusesByName` |
| Fig. 137 — PMI Entities data collection (p.168) | **none** | `9.5-only` | `spec` | **done** | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95Elements.kt:Pmi95Entities` | Thirteen typed collections, fixed order, all unconditional. Order confirmed twice: the figure's arrows on the rendered p.168 image, and §7.2.6.2.1.1 … .13. Test: `Pmi95DocumentTest.theThirteenTypedCollectionsAreReadInFigure137Order` (distinct counts per collection, so a permutation is visible) |
| Fig. 138 — PMI Dimension Entities data collection (p.168) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 Dimension Count` + `PMI 2D Data ×count` |
| Fig. 139 — PMI 2D Data collection (p.169) | Fig. 120 | `identical` | `spec+fixture` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi952dData`; v10 `MetaDataCodecs.kt:readPmi2dData` | `PMI Base Data`, `I32 Text Entity Count`, `2D Text Data ×count`, `Non-Text Polyline Data`. 9.5 **labels** the fourth box v10 draws empty → **DESIGN.md delta 35 upgraded from fixture-inferred to cited**. F-11 |
| Fig. 140 — PMI Base Data collection (p.170) | Fig. 121 | `structural` (guards) | `spec+fixture` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95BaseData`; v10 `MetaDataCodecs.kt:readPmiBaseData` | 9.5 draws both guards v10 lost: `2D-Frame Flag != 0` → 2D-Reference Frame (v10 prose only — **confirms** the v10 code from a drawn citation) and `PMI Version Number > 4` → `U8 Symbol Valid Flag` (no v10 counterpart). F-3. The p.170 prose says plain "Version Number", which is unsatisfiable for a 1-or-2 field — the figure is right, as at Figs. 147, 148, 153, 156, 162. Test: `Pmi95DocumentTest.everyPmiVersionGateChangesTheWireLayout` |
| Fig. 141 — 2D-Reference Frame data collection (p.171) | Fig. 122 | `identical` | `spec+fixture` | done | done (re-ser); n/a | `MetaDataCodecs.kt:readPmi2dReferenceFrame` (shared) | Three `CoordF32` |
| Fig. 142 — 2D Text Data collection (p.171) | Fig. 123 | `identical` | `spec+fixture` | done | done (re-ser); n/a | `MetaDataCodecs.kt:readPmi2dText` (shared) | `I32 String ID`, `I32 Font`, `I32 Reserved Field`, `F32 Reserved Field`, Text Box, Text Polyline Data. v10 renames the two reserved fields "Empty Field"; the model stores them (`emptyFieldI32`/`emptyFieldF32`). Font value set identical to v10 Table 62 |
| Fig. 143 — Text Box data collection (p.173) | Fig. 124 | `identical` | `spec+fixture` | done | done (re-ser); n/a | `MetaDataCodecs.kt:readPmiTextBox` (shared) | Six `F32`, same order |
| Fig. 144 — Constructing Text Polylines from data arrays (p.174) | Fig. 125 | `identical` (illustrative, non-normative) | `spec` | n/a | n/a | — | Same worked example |
| Fig. 145 — Text Polyline Data collection (p.174) | Fig. 126 | `identical` — and both **contradict a real producer** | `conflict` | done (both forms) | done (re-ser, byte-exact for both) | `MetaDataCodecs.kt:readPmiTextPolylineData` (shared), form at `Pmi95Elements.kt:Pmi95TextPolylineForm` | The `Polyline Segment Index Count > 0` guard encloses the index loop **and** the `VecF32` in 9.5 Fig. 145 *and* v10 Fig. 126 (both page images). NX 10.5 writes the empty vector regardless. **Corrects DESIGN.md delta 36**; `vertexCoords` is now nullable so both forms round-trip, and the 9.5 reader picks between them from the body's exact extent, naming the off-document one. F-4. Tests: `Pmi95DocumentTest.aZeroSegmentTextPolylineFollowsTheFigureByDefault`, `.theProducerFormIsAcceptedRecordedAndNamed`, `.theArbitrationStaysSilentWhenBothFormsAgree` |
| Fig. 146 — Constructing Non-Text Polylines from packed 2D data arrays (p.175) | Fig. 127 | `structural` (illustrative) | `spec` | n/a | n/a | — | 9.5's picture has an "Array of Polyline Type Values"; v10's adds an "Array of Polyline Width Values". Matches the wire delta below |
| Fig. 147 — Non-Text Polyline Data collection (p.176) | Fig. 128 | `structural` + `widths` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95NonTextPolylineData` | Three deltas: `I16` segment index (v10 `I32`); the `I32 Polyline Type Count` + `I16 Polyline Type` pair gated on `PMI Version Number > 4` (unconditional in v10); and **no** Polyline Width arrays at all (v10-only). `VecF32 Polyline Vertex Coords` unguarded in both. Guard extent read from the rendered p.176 image |
| Fig. 148 — PMI Note Entities data collection (p.177) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95Note` | `I32 Note Count` + per note `{PMI 2D Data, [PMI Version Number > 5] U32 URL Flag}` |
| Fig. 149 — PMI Datum Feature Symbol Entities (p.178) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 DFS Count` + `PMI 2D Data ×count` |
| Fig. 150 — PMI Datum Target Entities (p.178) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 Datum Target Count` + `PMI 2D Data ×count` |
| Fig. 151 — PMI Feature Control Frame Entities (p.179) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 FCF Count` + `PMI 2D Data ×count` |
| Fig. 152 — PMI Line Weld Entities (p.179) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 Line Weld Count` + `PMI 2D Data ×count` |
| Fig. 153 — PMI Spot Weld Entities (p.180) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95SpotWeld` | `I32 Spot Weld Count` + per weld `{PMI 3D Data, [PMI Version Number >= 4] CoordF32 Weld Point + three DirF32}` — 48 extra bytes when gated in. Presence recorded as a nullable `geometry` record, never re-derived on write |
| Fig. 154 — PMI 3D Data collection (p.181) | **none** | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi953dData`, model `Pmi953dData` | `PMI Base Data`, `I32 String ID`, `I16 Polyline Dimensionality` (2 or 3), `I32 Polyline Segment Index Count`, `I16 Polyline Segment Index ×count`, `VecF32 Polyline Vertex Coords`. **No `> 0` guard** — unlike Fig. 145. The dimensionality field is 9.5's answer to what v10 infers from "is it a Generic PMI Entity" |
| Fig. 155 — PMI Surface Finish Entities (p.182) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 SF Count` + `PMI 2D Data ×count` |
| Fig. 156 — PMI Measurement Point Entities (p.183) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95MeasurementPoint` | `I32 MP Count` + per point `{PMI 3D Data, [PMI Version Number >= 4] CoordF32 Location + three DirF32}` |
| Fig. 157 — PMI Locator Entities (p.184) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:read2dCollection` | `I32 Locator Count` + `PMI 2D Data ×count` |
| Fig. 158 — PMI Reference Geometry Entities (p.184) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities` | `I32 Reference Geometry Count` + `PMI 3D Data ×count`. The p.184 table derives the geometry kind from `Polyline Segment Index[1]` (`==1` point, `==2` polyline, `>2` polygon) — an interpretation rule, deliberately not imposed by the decoder |
| Fig. 159 — PMI Design Group Entities (p.185) | Fig. 111 | `structural` + `widths` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95DesignGroup` | 9.5: `I32 Design Group Count`, per group `I32 Group Name String ID` then **`[PMI Version Number >= 3]`** gating `I32 Attribute Count` + the attribute loop (guard extent from the page image). v10: `U32` counts, attributes unconditional. A PMI-version-2-or-lower group is 4 bytes where the v10 codec would read 8 — unreachable in a conforming 9.5 file (3 is the lowest documented PMI version), and the guard is implemented anyway |
| Fig. 160 — Design Group Attribute (p.186) | Fig. 112 | `identical` | `spec+fixture` | done | done (re-ser); n/a | `MetaDataCodecs.kt:readPmiDesignGroupAttribute` (shared by both generations) | `I32 Attribute Type` (1 Integer / 2 Double / 3 String), branch value, `I32 Label String ID`, `I32 Description String ID`. Value set = v10 Table 54 |
| Fig. 161 — PMI Coordinate System Entities (p.187) | none | `9.5-only` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Entities`, model `Pmi95CoordinateSystem` | `I32 Coord Sys Count` + per system `{I32 Name String ID, CoordF32 Origin, CoordF32 X-Axis Point, CoordF32 Y-Axis Point}` = 40 bytes each |
| Fig. 162 — PMI Associations (p.188) | Fig. 113 | `structural` (reorder + guard) | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Associations`, model `Pmi95Association` | 9.5: `Source Data, Destination Data, Reason Code, [PMI Version Number > 5] Source Owner, Destination Owner`. v10: `Source, Source Owner, Reason, Destination, Destination Owner`. Same five `I32` words, **permuted** — a v10-order read consumes exactly the right bytes and yields wrong meaning, so the test asserts semantics, not length. F-5, DESIGN.md delta 48. Tests: `Pmi95DocumentTest.associationsFollowTheNinePointFiveFieldOrderNotTheV10One`, `.associationOwnersAreOffTheWireBelowPmiVersionSix` |
| Table (unnumbered) — Source Data bit allocation (p.189) | Table 55 | `identical` | `spec` | done (accessors) | n/a | `Pmi95Elements.kt:Pmi95Association` | Bits 0-23 id, 24-30 type (22 values, 0…21), bit 31 indirect flag. Identical value list; carried as read, never validated — the set is open in both documents |
| Table (unnumbered) — Reason Code values (p.189) | Table 56 | `identical` | `spec` | done (carried) | n/a | — | Same 17 codes (0,1,2,5,10–17,72,73,98,99,100). Carried as read |
| Fig. 163 — PMI User Attributes (p.191) | Fig. 114 | `widths` | `spec+fixture` | done | done (re-ser); n/a | `MetaDataCodecs.kt:readPmiUserAttributes` (shared) | `I32 User Attribute Count` (v10 `U32`), then `I32 Key String ID` / `I32 Value String ID` per attribute — four bytes either way, a signedness label only. 9.5's figure draws both fields; v10's garbles the second box |
| Fig. 164 — PMI String Table (p.191) | Fig. 115 | `structural` (string type) | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95StringTable` | 9.5 writes `String : PMI String` — §7.1.1 (p.24) defines `String` as `I32 Count + Count × U8`; v10 writes `MbString` (`Count × U16`). Every PMI string in a 9.5 file is **single-byte**, and the v10 reading desynchronizes immediately. F-8, DESIGN.md delta 49. Test: `Pmi95DocumentTest.theStringTableAndFontNamesAreSingleByteStrings` |
| Fig. 165 — PMI Model Views (p.192) | Fig. 116 | `structural` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95ModelViews`, model `Pmi95ModelView` | Same eleven fields (76 bytes/view). v10 **appends** `I32 Property Count` + `PMI Property ×count` inside the per-view loop; 9.5 has neither — its per-view properties live in the Figure-136 tail (F-2). Count `I32` vs `U32`. Test: `Pmi95DocumentTest.modelViewsCarryNoPropertyListOfTheirOwn` |
| Fig. 166 — Generic PMI Entities (p.194) | Fig. 119 | `structural` (guard) + `widths` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95GenericEntities` | 9.5 gates `U16 User Flags` on `PMI Version Number > 6` (guard extent from the page image); v10 reads it unconditionally. Count `I32` vs `U32`. All other fields and their order identical |
| Table (unnumbered) — Generic PMI Entity Type values (pp.194–195) | Table 60 | `v10 superset` | `spec` | done (carried) | n/a | — | 9.5 lists 36 codes 0x0001…0x0128; v10 adds 16 more (0x0230 … 0x0308). No 9.5-only code. Neither set is validated by the code, correctly — the v10 fixture writes codes its own table omits |
| Table (unnumbered) — Generic PMI User Flag values (p.195) | Table 61 | `identical` | `spec` | done (carried) | n/a | — | Single documented bit `0x0001` "flat to screen only" |
| Fig. 167 — PMI Property (p.196) | Fig. 117 | `identical` | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95Property` | Key atom + value atom |
| Table 7 — Common Property Keys and Their Value Encoding formats (pp.196–197) | Table 58 | `identical` (spelling) | `spec` | n/a: interpretive | n/a | — | Same 18 keys; v10 spells the four colour keys `…Colour`, 9.5 `…Color`. Interpretive, not wire |
| Fig. 168 — PMI Property Atom (p.198) | Fig. 118 (retitled *Key PMI Property Atom*) | `structural` (guard) | `spec` | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95PropertyAtom`, model `Pmi95PropertyAtom` | `MbString Value` then **`[PMI Version Number > 6] U32 Hidden Flag`**. Three encodings now coexist across the library: absent (9.5, PMI ≤ 6), `U8` (NX 10.5, delta 32) and `U32` (documented v10) — recorded by a nullable field, never normalized. F-10, DESIGN.md delta 50. Test: `Pmi95DocumentTest.theHiddenFlagIsAbsentAtPmiVersionSixAndPresentAtSeven` |
| Fig. 169 — PMI CAD Tag Data (p.199) | Fig. 129 | `identical` | `spec` | done (indices + framing) | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95CadTagData` | `I32 CAD Tag Index Count`, indices, `Compressed CAD Tag Data`. The index-count formula lists the same fifteen entity counts in both documents — including the twelve v10 no longer defines, independent evidence that v10's chapter 11 is a deletion. **In 9.5 the formula is satisfiable and is enforced** (`Pmi95ManagerMetaDataElement.cadTagIndexCount`); the v10 sum of three kinds is a v10-only simplification. Test: `Pmi95DocumentTest.aCadTagIndexCountOutsideTheFormulaRefusesByName` |
| §8.1.16 Fig. 242/243 — Compressed CAD Tag Data (p.285) *(embedded dependency)* | Fig. 154 | `structural` + `widths` | `spec` | **framed, not decoded** (`CAD_TAG_VECTORS_UNRECOGNIZED`) | done (re-ser, byte-exact) | `Pmi95Codecs.kt:readPmi95CadTagData` | Out of package range but reached from Fig. 169, and genuinely different: `I16 Version Number` (v10 `U8`), an added `I32 CAD Tag Count` gating the whole body, `Int32CDP2`+`Lag1` vectors (v10: third-generation `Int32CDP`, no predictor), and the 64-bit tags as two `I32` halves (Fig. 243) instead of an `Int64CDP`. `encoding/CadTagData.kt` is **untouched** — the codec belongs to the compression package. The collection's own Data Length pins the extent (delta 34, whose wording 9.5 repeats), so the coded bytes are verbatim and the element still round-trips. Deferral recorded in DESIGN.md. Test: `Pmi95DocumentTest.cadTagDataIsFramedAndNamedRatherThanDecoded` |
| Fig. 170 — PMI Polygon Data (p.200) | Fig. 130 | `structural` | `spec` (the texture branch: `conflict` — figure vs its own prose) | done | done (re-ser); n/a | `Pmi95Codecs.kt:readPmi95PolygonData`, model `Pmi95PolygonData` | Wholly different shape: `I16 Version` + `I32 Reserved Field` (v10 `U8` + a declared element count), the element count is `vNumVerts`' own length, and the bindings + dimension are **inline per element** in the order Normal → Color → Texture where v10 hoists them into parallel vectors ordered Color → Normal → Texture. F-12. The figure labels the `NormalBinding == 1` box `VecF32: Vertices` (prose: Normals — same width, a naming slip) and the `TextureBinding == 1` box `I16 : Reserved Field` (prose: `VecF32: Texture Coords`). **The prose is followed and the deviation is named** by `PMI_POLYGON_TEXTURE_BINDING_UNSETTLED`; deferral recorded. Tests: `Pmi95DocumentTest.polygonDataIsTheNinePointFiveShapeWithInlineBindings`, `.theTextureBindingBranchFollowsTheProseAndSaysSo` |
| §7.2.7 PMI Data Segment (p.202) | Annex H | `identical` | `spec+fixture` | done (framing) | done (re-ser) | `MetaDataDocument.kt:150` | "a PMI Data Segment should be treated exactly the same as a 7.2.6 Meta Data Segment" — `isMetaDataSegment` unions Table-6 types 3 and 4 for all generations |
| §9.6 Metadata Conventions (pp.296–299) | §13 Metadata Conventions | `identical in kind` | `spec` | n/a: interpretive | n/a | — | Tables 9/10 and the tessellation/miscellaneous tables constrain property **keys** and their string encodings, not the Property Proxy byte layout. F-7 |

**Counts:** 45 rows. `identical` 16 · `widths` 3 · `structural` 12 · `9.5-only` 16 (14 layouts +
...
Evidence: `spec+fixture` 12, `spec` 31, `conflict` 1 (Fig. 145 — F-4, resolved by arbitration
with both forms round-tripping) + 1 partial (Fig. 170's texture branch, figure against its own
prose), `guess` 0.

Read: `done` 41 · `framed, not decoded` 1 · `n/a` 3. Write: byte-faithful re-serialization
everywhere it is read; authoring stays `n/a: writer emits v10` throughout.
...
| §8.1.3 Float64 Compressed Data Packet (Figs. 224–226, p.264) | **no v10 counterpart** — v10 replaced it with `Int64CDP` (§12.1.2) | `9.5-only` | `—` | `—` | **implemented by package P8** as `encoding.Float64Cdp`. Every 9.5 F64 vector in this package routes through it. Finding 2; package H's §8.1.3 rows |
| §8.1.13 Compressed Entity List for Non-Trivial Knot Vector (p.276) | v10 §12.1.13 | `structural` (predictor + packet generation; see package H's §8.1.13 row) | `done` (both generations since P8) | `done` (re-ser) | reached from Figs. 116 and 8.1.15.1 |
| §8.1.14 Compressed Control Point Weights Data (p.280) | v10 §12.1.14 | `structural` (predictor + both packet families; see package H's §8.1.14 row) | `done` (both generations since P8) | `done` (re-ser) | reached from Figs. 119 and 8.1.15.2 |
| §8.1.15 Compressed Curve Data (Figs. 238–241, p.282) | v10 §12.1.15, Figs. 150–153 | `structural`, four deltas: (a) the five `VecI32` are `Lag1` in 9.5, `NULL` in v10; (b) they are `Int32CDP` **Mk. 1** in 9.5, gen-3 in v10; (c) "NURBS Curve **Reserved** Fields" → "**Empty** Fields"; (d) Control Points and Knot Vectors are `VecF64{Float64CDP, NULL}` in 9.5, `VecF64{Int64CDP, NULL}` in v10 | `opaque` (V9 element) / `done` (v10) — the **collection** reads in both generations since package P8 (`CompressedCurveData.read95`); the JT 9 *element* that contains it stays opaque | `n/a: byte-faithful carry` | the whole 9.5 wireframe curve payload lives here. Findings 2, 3, 9 |
...
The §8.1 table carries the **Evidence** column the other packages use (added by package P8, which
flipped most of its `—` rows); the §8.2, §9 and Appendix tables are algorithm-granularity and keep
the original six columns.

### §8.1 Common Compression Data Collection Formats

| 9.5 unit | v10 counterpart | Delta | Evidence | Read | Write | Code | Notes |
|---|---|---|---|---|---|---|---|
|  `§8.1.1 / Fig. 218 — Int32 Compressed Data Packet (p.254)` | `Fig. 132` (v10 has only one Int32 packet, and it is the Mk. 2 lineage) | `structural` — a different packet from Mk. 2 end to end. Mk. 1 order: `U8 CODEC Type` → *(Arithmetic)* `Int32 Probability Contexts` → `I32 Out-Of-Band Value Count` → *(count>0)* nested `Int32 Compressed Data Packet` → *(codec≠Null)* `I32 CodeText Length`, `I32 Value Element Count` → *(table count>1)* `I32 Symbol Count` → `VecU32 CodeText`. **No leading Value Count** (so no "empty packet" form), OOB gated on an explicit count, CodeText is a `VecU32` (own `I32` count, p.21) not a bare word run, and a separate Symbol Count exists for the 2-table case. | `spec` | `done` | `done` (byte-faithful re-serialization) | `Int32CdpMk1.kt:51`, `:142`, `:179` | Used by every `VecI32{Int32CDP,…}` field: all JT B-Rep topology streams, LWPA, §8.1.13/14/15. Finding 2. **Package P8 implements it** (`Int32CdpMk1`), deliberately as a type unrelated to `Int32Cdp`: nothing in the bytes tells the generations apart, so the reading call site chooses and a caller that guesses is a bug. CODEC 4 (Chopper) is refused by name — Figure 218 draws no chopper fields and §8.1.2 introduces the Chopper with Mk. 2. Tests: `Int32CdpMk1Test`, `CompressedDataPacketFixtureTest.everyArithmeticPacketDecodesTheSameWhenReframedAsMk1` (663 real Mk. 2 packets re-framed) and `.noMk2PacketIsSilentlyReadAsMk1` (684 named refusals). |
|  `§8.1.1.1 / Fig. 219 — Int32 Probability Contexts (p.256)` | `Fig. 133` (single table, no count byte) | `structural` — leading `U8 Probability Context Table Count` (1 or 2); per table `U32{32} Entry Count`, `U32{6} Number Symbol Bits`, `U32{6} Number Occurrence Count Bits`, **first table only** `U32{6} Number Value Bits`, `U32{6} Number Next Context Bits`, `U32{32} Min Value`, entries, `U32{variable} Alignment Bits`. v10: no table count, `U32{16}` entry count, no symbol-bits, no next-context-bits, `U32{7}` value bits. | `spec` | `done` | `done` (raw block preserved) | `Int32CdpMk1.kt:291`, `:325` | Note the 32-bit entry count (Mk. 2 and v10 use 16). Finding 2. Implemented by P8, both the one- and two-table forms, with the second table reusing the first's Number Value Bits *and* Min Value (Figure 219 draws neither in the second-table branch). Tests: `Int32CdpMk1Test`, `CompressedDataPacketFixtureTest.everyArithmeticPacketDecodesTheSameWhenReframedAsMk1` (663 real Mk. 2 packets re-framed) and `.noMk2PacketIsSilentlyReadAsMk1` (684 named refusals). |
|  `§8.1.1.1.1 / Fig. 220 — Int32 Probability Context Table Entry (p.257)` | `Fig. 134` | `structural` — `Symbol` (stored +2; `−2` = escape), `Occurrence Count`, `Associated Value` (−Min), **`Next Context`**. v10 replaces `Symbol` with a `U32{1}` escape flag and drops `Next Context`. | `spec` | `done` | `done` | `Int32CdpMk1.kt:303`, `:426` | The `Next Context` field is what makes Mk. 1's decoder a *state machine*; Mk. 2's is not. Finding 2. Implemented by P8; `Next Context` drives the per-symbol context switch of App. C §3.1 and is validated against the table count. `spec unclear`: §8.1.1 says an escape met outside table 0 does *not* spend an out-of-band value and never says what it does instead — the reader emits nothing, the only reading under which "the number of Symbols decoded can be larger than the number of Values produced" holds, and the Value Element Count check is the net. See DESIGN.md. |
|  `§8.1.2 / Fig. 221 — Int32 Compressed Data Packet Mk. 2 (p.259)` | `Fig. 132` | `structural` (small, but real) — order `I32 Value Count` → *(count>0)* `U8 CODEC Type` → chopper: `U8 Chop Bits`, *(Chop Bits ≠ 0)* `I32 Value Bias` + `U8 Value Span Bits` + MSB/LSB nested packets; else `I32 CodeText Length` + `CodeText Length/32` × `U32 CodeText Word`; arithmetic then `Int32 Probability Contexts Mk. 2` + **an unconditional nested `Int32 CDP Mk. 2 : OOB Data Values`**. v10 adds CODEC 5 (Move-to-Front) + its two window packets, forbids Chop Bits 0 ("always greater than 0, and less than 32"), conditions the OOB field on external compression, and raises the recursion cap from **3** ("For JT v9 files, the maximum recursion depth may not exceed three", p.258) to eight. | `spec+fixture` | `done` | `done` (byte-faithful) | `Int32Cdp.kt:191`, `:206`, `:237` | Codec set `{0,1,3,4}` — matches `CodecDriver2::CodecType` (App. C §1.6, p.318). Reader caps depth at 8, not 3 — lenient, harmless. Findings 4, 5.  |
|  `§8.1.2.1 / Fig. 222 — Int32 Probability Contexts Mk. 2 (p.261)` | `Fig. 133` | `structural` — `U32{16} Entry Count`, `U32{6} Number Symbol Bits`, `U32{6} Number Occurrence Count Bits`, `U32{6} Number Value Bits`, `U32{32} Min Value`, entries, `U32{variable} Alignment Bits`. v10 drops Number Symbol Bits and widens Number Value Bits to `U32{7}`. Header is **66 bits in 9.5 vs 61 in v10**; they diverge at bit 22. Single table only (the Mk. 1 table-count byte is gone). | `spec+fixture` | `done` | `done` (raw block preserved) | `Int32Cdp.kt:482`, `:500` | Exactly DESIGN.md delta 16 — now a citation. The prose's leftover "*If a second Probability Context Table is present…*" is a Mk. 1 carry-over; Fig. 222 has no table count. Finding 1.  |
|  `§8.1.2.1.1 / Fig. 223 — Int32 Probability Context Table Entry Mk. 2 (p.262)` | `Fig. 134` | `structural` — `U32{Number Symbol Bits} Symbol` (+2 bias, `−2` = escape), `U32{Number Occurrence Count Bits} Occurrence Count`, `U32{Number Value Bits} Associated Value` (−Min). **No Next Context.** v10: `U32{1} Is Escape Symbol`, occurrence count, `I32{…} Associated Value`. | `spec+fixture` | `done` | `done` | `Int32Cdp.kt:489`, `:512` | Escape detection `symbol == -2` at `Int32Cdp.kt:263` matches `CntxEntryBase2::CEBEscape = -2` (App. C §1.2, p.310).  |
|  `§8.1.3 / Fig. 224 — Float64 Compressed Data Packet (p.264)` | `none` (v10 has `Int64 CDP`, Fig. 135 — a different mechanism) | `9.5-only` — `U8 CODEC Type` → *(codec≠Null)* `Float64 Probability Contexts`, `F64 Value Range Min`, `F64 Value Range Max`, `I32 Out-Of-Band Value Count`, `VecF64 Out-Of-Band Values`, `I32 CodeText Length`, `I32 Value Element Count` → *(table count>1)* `I32 Symbol Count` → `VecU32 CodeText`. Out-of-band is **always raw `VecF64`, never nested** ("the Float64 Compressed Data Packet simply writes out the 'out-of-band data' array with no additional encoding attempted", p.263). No chopper, no bit-packed context. | `spec` | `done` | `done` (byte-faithful re-serialization) | `Float64Cdp.kt:35`, `:122`, `:158` | Finding 3. **Package P8 implements it.** CODEC 1 (Bitlength) and CODEC 4 (Chopper) are refused by name: App. C §2.1's bitlength codec is Int32-valued and §8.1.3 defines no 64-bit form or reinterpretation step, and Figure 224 draws no chopper fields. `spec unclear`: the Null CODEC's `VecU32` packing is undocumented — two words per value is forced by the framing (a Null packet has no Value Element Count), the low-word-first order is taken by analogy from v10 §12.1.2. Tests: `Float64CdpTest`. |
|  `§8.1.3.1 / Fig. 225 — Float64 Probability Contexts (p.266)` | `none` (`Fig. 136` is Int64, bit-packed) | `9.5-only` — plain `I32 Probability Context Table Count` (1 or 2), then per table `I32 Probability Context Table Entry Count` + entries. **Byte-aligned `I32`s, no bit packing, no Min Value, no field widths, no alignment bits.** | `spec` | `done` | `done` | `Float64Cdp.kt:249`, `:290` | `spec unclear`: §8.1.3.1 never says the collection is Arithmetic-only (its Int32 siblings do), and Fig. 224's guard is "CODEC Type not equal to Null". A Bitlength Float64 packet would then still carry a context. Finding 3. Implemented by P8 field by field (no bit packing, so no raw block). The `spec unclear` above is resolved conservatively: a two-table packet is read, but decoding one with a live CodeText is **refused by name** — Figure 226 has no Next Context field, so there is nothing to switch tables with. With CodeText Length 0 the same packet stays readable. |
|  `§8.1.3.1.1 / Fig. 226 — Float64 Probability Context Table Entry (p.266)` | `none` (`Fig. 137`) | `9.5-only` — `I32 Symbol` (`−2` = escape, stored **unbiased**, unlike the Int32 tables' +2), `I32 Occurrence Count`, `F64 Associated Value`, `I32 Reserved Field`. 20 bytes flat. | `spec` | `done` | `done` | `Float64Cdp.kt:266`, `:290` | The `I32 Reserved Field` must be carried verbatim for round-tripping (§9.3). Finding 3. Implemented by P8; the `I32 Reserved Field` is carried verbatim and asserted by `Float64CdpTest.theReservedFieldOfEveryContextEntryIsCarriedVerbatim`. |
|  `§8.1.4 / Fig. 227 — Compressed Vertex Coordinate Array (p.267)` | `Fig. 138` | `structural` — lossless branch stores **two packets per component** (`VecU32{Int32CDP2,Lag1} Vertex Coord Exponents` then `… Mantissae`, float bits = `exp<<23 \| mantissa`); v10 stores one `Binary Vertex Coords` packet per component. Header (`I32 Unique Vertex Count`, `U8 Number Components`, `Point Quantizer Data`) and trailer (`I32 Vertex Coordinate Hash`) identical; quantized branch identical. v10 adds "the only legal value for [Number Components] is 3". | `spec+fixture` | `done` | `done` | `VertexArrays.kt:89` | Confirms DESIGN.md delta 19. Hash pseudo-code is **per value** on the lossless path here and the code matches — see finding 6 (v10's bytes disagree with its identical pseudo-code; 9.5's do not).  |
|  `§8.1.5 / Fig. 228 — Compressed Vertex Normal Array (p.269)` | `Fig. 139` | `structural` — quantized branch stores **four packets** (`Sextant`, `Octant`, `Theta`, `Psi` Codes, all `VecU32{Int32CDP2}`, NULL predictor); v10 stores one packed `Deering Normal Codes` packet. Lossless branch: exponent+mantissa pair per component vs v10's single binary packet. v10 adds "The maximum value for this field is 13"; **9.5 states no maximum** (and §8.2.4 implies 6 — see finding 8). Hash is `U32` in both. | `spec+fixture` | `done` | `done` | `VertexArrays.kt:234` | Confirms DESIGN.md delta 19 second half. The `quantizationBits > 13` refusal at `VertexArrays.kt:270` is a v10 rule imported into the JT 9 path — harmless (9.5 files use ≤ 8) but not 9.5-derived.  |
|  `§8.1.6 / Fig. 229 — Compressed Vertex Texture Coordinate Array (p.271)` | `Fig. 140` | `structural` — `I32 Texture Coord Count` (v10 `U32`); lossless branch two packets per component (exponents, mantissae) vs v10's one binary packet; quantized branch identical (`Texture Quantizer Data` then one `VecU32{Int32CDP2,Lag1}` per component); hash `U32` (v10 `I32`). | `spec` | `—` (JT 9 refuses the binding: `ShapeLodDocument.kt:408`) | `n/a: never read` | `—` | 9.5 doc error: the figure prints the guard **"QuantBits = 0" on both branches** (verified in the PDF, p.271); the right branch must be `> 0`. Finding 10.  |
|  `§8.1.7 / Fig. 230 — Compressed Vertex Color Array (p.273)` | `Fig. 141` | `structural` — quantized branch stores **four separate code packets** (`Hue/Red`, `Sat/Green`, `Value/Blue`, `Alpha`, each `VecU32{Int32CDP2,Lag1}`); v10 stores **one packed `Colour Codes`** packet with bit fields. Lossless branch exp+mantissa pairs vs v10 binary. `I32 Color Count` (v10 `U32`), hash `U32` (v10 `I32`). Hash chains four arrays (v10 hashes one). | `spec` | `—` (binding refused) | `n/a: never read` | `—` | Same "QuantBits = 0" twice figure error as Fig. 229 (PDF p.273). Finding 10.  |
|  `§8.1.8 / Fig. 231 — Compressed Vertex Flag Array (p.275)` | `Fig. 142` | `widths` — `I32 Vertex Flag Count` (v10 `U32`, same 4 bytes, signedness only); `VecU32{Int32CDP2} Vertex Flags` vs `VecU32{Int32CDP}`. Structure otherwise identical; no hash in either. | `spec` | `—` (JT 9 refuses bit 6 of Vertex Bindings) | `n/a: never read` | `VertexArrays.kt:381` is v10-only (`readInt32CdpValuesV10`) | Adding the JT 9 form is a two-line change (`Int32Cdp.read` + `readI32`); the blocker is the binding-mask refusal at `ShapeLodDocument.kt:408`, not the array. Finding 9.  |
|  `§8.1.9 / Fig. 232 — Point Quantizer Data (p.275)` | `Fig. 144` | `identical` — X, Y, Z Uniform Quantizer Data, in that order, nothing else. | `spec+fixture` | `done` | `done` | `VertexArrays.kt:43` |  |
|  `§8.1.10 / Fig. 233 — Texture Quantizer Data (p.276)` | `Fig. 145` | `identical` — *n* Uniform Quantizer Data, *n* from the enclosing array's `U8 Number Components`; count not stored. | `spec` | `—` | `n/a: never read` | `—` | Unimplemented in both generations (no texture-coordinate consumer).  |
|  `§8.1.11 / Fig. 234 — Color Quantizer Data (p.277)` | `Fig. 146` | `identical` — `U8 HSV Flag`; `=0` → Red/Green/Blue/Alpha Uniform Quantizer Data; `=1` → `U8` Hue/Saturation/Value/Alpha bit counts. The HSV assumed ranges table (Hue 0–6, others 0–1) is identical (9.5 p.276 / v10 Table 66). | `spec` | `—` | `n/a: never read` | `—` | Unimplemented in both generations.  |
|  `§8.1.12 / Fig. 235 — Uniform Quantizer Data (p.278)` | `Fig. 147` | `identical` — `F32 Min`, `F32 Max`, `U8 Number Of Bits`, range `[0,32]`. | `spec+fixture` | `done` | `done` | `VertexArrays.kt:18` |  |
|  `§8.1.13 / Fig. 236 — Compressed Entity List for Non-Trivial Knot Vector (p.279)` | `Fig. 148` | `structural` (predictor) — four `VecI32{Int32CDP, `**`Stride1`**`} : Entity Index Codes`; v10 uses `{Int32CDP, `**`Lag1`**`}`. Also Mk. 1 vs the v10 packet. The `VecI32 Entities of Knot Type Exist Flags` header and the four Table-68 categories are identical. 9.5's trivial-knot Case-2 carries an extra clause v10 dropped: "All distinct interior knots are repeated exactly one time" (p.278). | `spec` | `done` | `done` (byte-faithful re-serialization); `n/a: writer emits v10` | `CurveData.kt:96` (`read95`, Mk. 1 + `Predictor.STRIDE1`) | The reconstruction sketch (`case 0..3` numVals formulas) is byte-identical in the two documents. Finding 2. Opened by P8: the collection is generation-independent in shape, so `read`/`read95` share one body and differ only in the packet and predictor they are handed. Tests: `CurveData95Test`. |
|  `§8.1.14 / Fig. 237 — Compressed Control Point Weights Data (p.281)` | `Fig. 149` | `structural` — `I32 Weights Count`, then `VecI32{Int32CDP, `**`Stride1`**`} : Weight Indices` (v10: `{Int32CDP, `**`Lag1`**`}`), then `VecF64{`**`Float64CDP`**`, NULL} : Weight Values` (v10: `VecF64{Int64CDP, NULL}` with bitwise `I64`→`F64` reinterpretation). The "weight 1 is not stored, infer it" rule is identical in both. | `spec` | `done` | `done` (byte-faithful re-serialization); `n/a: writer emits v10` | `CurveData.kt:166` (`read95`) | The Float64CDP dependency makes this unreachable without finding 3's work. Opened by P8; the `VecF64{Float64CDP, NULL}` weight values are `Float64CdpVector` (`Vectors.kt:92`), natively F64 with no reinterpretation step. Tests: `CurveData95Test`. |
|  `§8.1.15 / Fig. 238 — Compressed Curve Data (p.282)` | `Fig. 150` | `structural` — all five per-curve index vectors are `VecI32{Int32CDP, `**`Lag1`**`}`; v10 uses `{Int32CDP, `**`NULL`**`}`. Fifth vector named `NURBS Curve Reserved Fields` (v10 `NURBS Curve Empty Fields`). `VecF64{`**`Float64CDP`**`, NULL} : NURBS Curve Knot Vectors` vs v10's `Int64CDP`. Sub-collection order and the Table-69/70/71 semantics identical. | `spec` | `done` | `done` (byte-faithful re-serialization); `n/a: writer emits v10` | `CurveData.kt:322` (`read95`) | Both the Mk. 1 packet and Float64CDP are prerequisites. Findings 2, 3. Opened by P8. The five per-curve vectors are `Lag1` here against v10's NULL — asserted, not assumed, by `CurveData95Test.theFivePerCurveVectorsAreLag1PredictedOnTheWire`. The *element* that contains this collection (the JT 9 Wireframe Rep Element, Fig. 130) stays opaque: its surrounding layout is unestablished and no fixture carries one. Tests: `CurveData95Test`. |
|  `§8.1.15.1 / Fig. 239 — Non-Trivial Knot Vector NURBS Curve Indices (p.284)` | `Fig. 151` | `identical` — a bare delegation to §8.1.13 (which itself differs; see that row). | `spec` | `done` | `done`; `n/a: writer emits v10` | `CurveData.kt:96` |  Delegates to §8.1.13's 9.5 form. |
|  `§8.1.15.2 / Fig. 240 — NURBS Curve Control Point Weights (p.284)` | `Fig. 152` | `identical` — a bare delegation to §8.1.14. | `spec` | `done` | `done`; `n/a: writer emits v10` | `CurveData.kt:166` |  Delegates to §8.1.14's 9.5 form. |
|  `§8.1.15.3 / Fig. 241 — NURBS Curve Control Points (p.284)` | `Fig. 153` | `structural` — `VecF64{`**`Float64CDP`**`, NULL} : Control Points`; v10 `VecF64{Int64CDP, NULL}` plus "each deserialized 64 bit integer number should be converted to bit wise equivalent 64 bit floating number" — a step 9.5 does not have because its codec is natively `F64`. | `spec` | `done` | `done`; `n/a: writer emits v10` | `CurveData.kt:322`, `Vectors.kt:92` | Finding 3. Opened by P8. 9.5 needs no bit-reinterpretation step here because `Float64CDP` symbols are already doubles — the v10 sentence about converting each `I64` has no 9.5 counterpart, and `Float64CdpVector` has no conversion to match it. |
|  `§8.1.16 / Fig. 242 — Compressed CAD Tag Data (p.285)` | `Fig. 154` | `structural` + `widths` — `I16 Version Number` (v10 `U8`); `I32 Data Length`; `I32 Version Number` ("1" the only valid value; v10 defers to local-version conventions); **`I32 CAD Tag Count` — a 9.5-only field**; then *(CAD Tag Count > 0)* `VecI32{Int32CDP2, `**`Lag1`**`} : CAD Tag Types` (v10 `{Int32CDP}`, NULL); *(Type-1 tags exist)* `VecI32{Int32CDP2, Lag1} : CAD Tags Type-1`; *(Type-2 tags exist)* the **`Compressed CAD Tag Type-2 Data` sub-collection** (§8.1.16.1) instead of v10's single `VecI64{Int64CDP}`. | `spec` | `—` | `n/a: writer emits v10` | `CadTagData.kt:63`, `:122` are v10-only | Two 9.5 figure errors here — the Type-1 branch box literally reads `I16:Version Number` (the prose says the tag vector), and both branch guards say "exist in `I32 : Surface Count` data", a copy-paste from the B-Rep. Prose is authoritative. Finding 10.  |
|  `§8.1.16.1 / Fig. 243 — Compressed CAD Tag Type-2 Data (p.286)` | `none` (v10 folds this into `VecI64{Int64CDP}`) | `9.5-only` — `VecI32{Int32CDP2, Lag1} : First I32 of Type-2 CAD Tags` then `VecI32{Int32CDP2, Lag1} : Second I32 of Type-2 CAD Tags`: 64-bit tags split into **two Int32 Mk. 2 packets**. | `spec` | `—` | `n/a: writer emits v10` | `—` | `spec unclear`: the document never says which half is high-order. Reader must resolve it from tag plausibility or refuse. Finding 2.  |
...
|  `§8.2.3 Arithmetic CODEC (p.288–293)` | `§12.2.3` | `identical` at prose level (both are the same tutorial: Shannon/Elias history, the `{2,9,12,…}` worked example, the 5-digit register table, the underflow discussion). The *implementations* differ by generation, not by document: Mk. 1 (App. C §3.1) is a **multi-context state machine** — it loops `numSymbolsToRead()` (= Symbol Count when two tables), switches context per symbol via `pCntxEntry->iNextCntx`, and only emits an out-of-band value when the escape arrives *in context 0*. Mk. 2 (App. C §3.2) has a single context, loops `nValues`, and emits an OOB value on every escape. The 16-bit renormalisation core (`_low=0`, `_high=0xffff`, `_code` = first 16 bits, `rescaledCode = ((code−low+1)*total−1)/(high−low+1)`, the `0x4000` underflow squeeze, two flush bits) is **identical in Mk. 1, Mk. 2 and v10**. | `done` (both forms) | `done` | `Int32Cdp.kt:713` (Mk. 2) · `Int32CdpMk1.kt:478` (Mk. 1) | Finding 4 — this corrects DESIGN.md delta 17. **Package P8 implements the Mk. 1 prefix-code scheme this prose describes** (`decodeBitlengthMk1`), from App. C §2.1: `cStepBits = 2`, the terminator is the first bit contradicting the previous increment bit, the loop is bounded by CodeText Length rather than a value count, and the two zero-width corner cases (a 0-bit field emits a zero consuming no bit; a stream ending in the accumulate state with width 0 emits one more) are reproduced. Tests: `Int32CdpMk1Test.bitlengthWalksThePrefixCodeWithTwoBitSteps`, `.bitlengthZeroWidthFieldsEmitZerosWithoutConsumingBits`. |
|  `§8.2.4 Deering Normal CODEC + Fig. 244 Sextant Coding on the Sphere (p.293)` | `§12.2.4 + Fig. 155` | `identical` — word-for-word, same sextant code assignment figure, same θ/φ formulas, same "n is in the range from 0 to 6 bits" and "max grand total of 18 bits (3+3+6+6)". | `done` (both drivers) | `done` | `Int32Cdp.kt:936` (shared core), `:1018` (single-context) · `Int32CdpMk1.kt:426` (multi-context) | The Kotlin underflow test `low & 0x4000 && !(high & 0x4000)` is the *commented-out* variant in App. C §3.2; given the preceding "top bits match" test failed, it is provably equivalent to the live `((_low>>14)==1)&((_high>>14)==2)`. No divergence. Finding 1. **P8 split the core out** as `ArithmeticDecoder`, one symbol per call against the histogram it is handed, so Mk. 1's per-symbol context switch and Mk. 2's single context are the *same* renormalisation code — which is what makes the 663-packet re-framing differential test meaningful. |
...
|  `App. C §1.2/1.4/1.6 — CntxEntry 2, ProbabilityContext 2, CodecDriver 2 (pp. 310–319)` **new in Rev-D** | `Annex B` (v10 reference source) | `structural` — `CEBEscape = −2`; `ProbContext2::lookupEntryByCumCount` accumulates counts in **file entry order** (`accumulateCounts`, entry 0 cumulative 0); `CodecDriver2::CodecType` = `{0 Null, 1 BitLength, 3 Arithmetic, 4 Chopper}` — **no Move-to-Front**; predictor enum of **12** values, adding `PredMean2=9`, `PredMean3=10`, `PredMean4=11` on top of 9.5 §6's table of 9. | `done` | `done` | `Int32CdpMk1.kt:291`, `:426` | The Mk. 1 decoder contract. Finding 2. Implemented by package P8; `lookupEntryByCumCount` is taken to accumulate counts in **file entry order**, as App. C §1.4 prints for `ProbContext2` — §1.3 declares the Mk. 1 method without a body, so this is the one place the Mk. 1 contract is filled in from its Mk. 2 sibling. |
| `App. C §2.1 — BitLengthCodec 1 (p.320)` | `none` | `9.5-only` — the prefix-code scheme of §8.2.2. | `—` | `n/a` | `—` | Needed only with Mk. 1. |
|  `App. C §2.2 — BitLengthCodec 2 (pp. 322–324)` **new in Rev-D** | `Annex B BitLengthCodec` | `structural` — 1-bit mode tag; **fixed**: `U{6} cBitsInMinSymbol`, `U{6} cBitsInMaxSymbol`, `S{cBitsInMinSymbol} iMinSymbol`, `S{cBitsInMaxSymbol} iMaxSymbol`, field width `_nBitsInSymbol(max−min)`, then unsigned fields biased by min; **variable**: `S{32} iMean`, `U{3} cBlkValBits`, `U{3} cBlkLenBits`, then per run a `S{cBlkValBits}` width delta repeated while it equals ±the extreme, a `U{cBlkLenBits}` run length, and `S{cCurFieldWidth}` fields biased by the mean. v10's Annex B uses nibble-coded min/max/mean and fixed 4-bit block/length widths. | `done` | `done` | `Int32CdpMk1.kt:478` | Needed only with Mk. 1, and implemented with it by package P8 — statement for statement, including the closing zero-width clause. |
| `App. C §3.1 — ArithmeticCodec 1 (p.325)` | `none` | `9.5-only` — see the §8.2.3 row. | `—` | `n/a` | `—` | |
|  `App. C §3.2 — ArithmeticCodec 2 (pp. 327–328)` **new in Rev-D** | `Annex B ArithmeticCodec` | `identical` core, different context type. Signature `decode(nValues, vOOBValues, vCodeText, nBitsCodeText, ovValues, pProbCntx)` — the OOB array is a **plain `Veci` handed in**, consumed one per escape, in order. | `done` | `done` | `Int32CdpMk1.kt:426` | Implemented by package P8: the loop runs over `numSymbolsToRead()` (Symbol Count with two tables, Value Element Count with one), the next context is the decoded entry's `iNextCntx`, and an escape spends an out-of-band value only in table 0. |
...
**What package P8 closed (2026-08-02).** Findings 2 and 3 — the Int32 CDP **Mk. 1** packet
(§8.1.1 with its probability-context list, its prefix-code bitlength codec and its multi-context
arithmetic driver) and the **Float64 CDP** (§8.1.3) — are implemented, and with them the
§8.1.13/§8.1.14/§8.1.15 collections in their 9.5 form. Twelve §8.1 rows and three Appendix C rows
flip to `done`, and the two §8.2 algorithm rows widen from one generation to both.

Every one of them is `spec`, **never** `spec+fixture`: no file in the corpus carries either
packet, because none carries a JT B-Rep, a ULP or a JT 9 wireframe segment.
`CompressedDataPacketFixtureTest` is the hook that fires when one arrives, and meanwhile does the
strongest thing the corpus permits — re-frames 663 real Mk. 2 arithmetic packets into the Mk. 1
layout around their own CodeText and requires identical values, and confirms that all 684 of them
are refused by name when offered to the Mk. 1 reader in their own framing.

Three things were deliberately **not** opened, and their notes say why rather than going quiet:
the JT 9 Wireframe Rep Element (Fig. 130 — surrounding layout unestablished, and
`wireframe/WireframeDocument.kt`'s own comment misnames the blocker as "Mk. 2 packets" where
finding 2 shows it is Mk. 1 + Float64CDP), the JT B-Rep topology streams (§7.2.3.1, package G's
rows), and the ULP `params1` collection (§7.2.2.2.2.1). Decoding a packet is not decoding the
element that contains one; those elements now lack only their own framing, not a codec.

One correction to finding 2 while implementing it: it lists "the JT LWPA element (Compressed
Types, params1 Codes)" among the Mk. 1 users. The **LWPA element is not** — 9.5 Fig. 216 writes
`VecI32{Int32CDP2, Lag1}` and `{Int32CDP2, NULL}`, i.e. Mk. 2, which is what package P7 implemented
and what `LwpaDocument` reads. The Mk. 1 `params1` codes are the *ULP*'s (§7.2.2.2.2.1, Fig. 100).
