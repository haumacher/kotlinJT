...
| Fig. 120 — PMI 2D Data collection (p.142) | done | done | MetaDataDocumentTest.pmiManagerDecodesEverySubCollectionAndRoundTrips, MetaDataFixtureTest | **delta 35**: the figure's unlabeled last box is Non-Text Polyline Data — fixture-confirmed, and since package P9 also **cited**: 9.5's Figure 139 (p.169) labels the same box |
...
| Fig. 126 — Text Polyline Data collection (p.146) | done | done | MetaDataDocumentTest.pmiManagerDecodesEverySubCollectionAndRoundTrips, MetaDataFixtureTest, Pmi95DocumentTest | **delta 36** (reworded by P9): the figure's `> 0` guard encloses the coordinate `VecF32` too, and NX 10.5 writes it anyway — a producer-vs-document conflict, not a documentation gap. `PmiTextPolylineData.vertexCoords` is nullable so both forms round-trip; the v10 reader keeps the producer form |
